...
## GEO search providers

GEO monitoring uses only grounded search responses. Configure provider secrets
in the Dolphin control service `.env`; do not copy them into the desktop app:

- Doubao uses `CONTROL_PROVIDER_API_KEY` plus `MODEL_GEO_DOUBAO`.
- OpenAI uses `CONTROL_GEO_OPENAI_API_KEY` plus `MODEL_GEO_OPENAI`.
- Perplexity uses `CONTROL_GEO_PERPLEXITY_API_KEY` plus `MODEL_GEO_PERPLEXITY`.
- Gemini uses `CONTROL_GEO_GEMINI_API_KEY` plus `MODEL_GEO_GEMINI`.

An unconfigured provider remains unavailable in `/api/v1/models/capabilities`
and is shown as “待接入” by the desktop app. It never falls back to an ordinary
chat completion. DeepSeek is intentionally excluded from unattended API jobs;
the desktop app offers a user-triggered, visible consumer-surface spot check.
