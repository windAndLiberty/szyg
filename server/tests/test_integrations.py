...
class TestWhisperClient:
    ...
    async def test_whisper_transcribe(self, mock_whisper_server: respx.MockRouter, config: MagicMock):
        ...
        async with AsyncClient() as client:
            # Act - fixture already mocks the endpoint
    ...
    async def test_whisper_retry(self):
        """
        ...
        # Act - Simulate retry logic using isolated respx mock context
        with respx.mock:
            respx.post("http://localhost:9000/transcribe").mock(side_effect=side_effect)
...
class TestClientCommonBehaviors:
    ...
    async def test_client_timeout(self):
        ...
        # Arrange
        with respx.mock:
            respx.get("http://slow-service/api").mock(
                side_effect=ConnectError("Connection timed out")
            )
            ...
            async with AsyncClient(timeout=0.1) as client:
                ...
                assert (
                    ...
                    or "Connect" in str(exc_info.value)
