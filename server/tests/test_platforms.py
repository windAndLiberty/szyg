...
class TestAntiDetect:
    ...
    def test_native_headless_config_avoids_obsolete_field_trial(self):
        from szyg.platforms.anti_detect import get_launch_config_native
        config = get_launch_config_native(headless=True)
        assert not any(arg.startswith("--force-fieldtrials=") for arg in config["args"])
