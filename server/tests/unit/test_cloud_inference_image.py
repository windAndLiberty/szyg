...
class TestCloudInferenceImageSizeNormalization:
    ...
    def test_reference_images_are_forwarded_to_seedream(self, client):
        sent = {}
        ...
        references = ["https://gateway.test/reference-1.png", "https://gateway.test/reference-2.png"]
        with respx.mock:
            respx.post(URL).mock(side_effect=capture)
            asyncio.run(client.generate_image("参考这些产品图", reference_images=references))

        assert sent["body"]["payload"]["image"] == references
