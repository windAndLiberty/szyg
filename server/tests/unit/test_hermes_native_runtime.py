...
def test_hermes_build_installs_szyg_runtime_dependencies():
    script = (ROOT / "electron" / "scripts" / "build-hermes-runtime.ps1").read_text(encoding="utf-8")
    requirements = (ROOT / "server" / "hermes-runtime-requirements.txt").read_text(encoding="utf-8")
    assert "hermes-runtime-requirements.txt" in script
