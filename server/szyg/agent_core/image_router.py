...
class ImageRouter:
    ...
    def volcano(self):
        if self._volcano is None:
            ...
            self._volcano = VolcanoEngineImageClient()
    ...
    async def generate(
        ...
        size: str = "1024x1024", backend: str | None = None,
    ) -> dict:
        """智能图像生成。LLM增强prompt + 火山引擎优先 + ModelScope降级。"""
        intent = await self._analyze_intent(user_input, style)
        chosen = backend or self._select_backend()

        if chosen == "volcano_engine":
            try:
                paths = await self.volcano.generate(
...
只返回 JSON。"""}], max_tokens=500)
    ...
    def _select_backend(self) -> str:
        # 默认优先使用火山引擎，不再探测本地 ComfyUI
        import os
        if os.environ.get("VOLCANO_ENGINE_API_KEY"):
            return "volcano_engine"
        return "modelscope"
