...
def _build_cache() -> None:
    ...
    for div_id in RETAINED_DIVISIONS:
        ...
        info = {
            ...
            "label_en": meta.get("label", div_id),
    ...
    for div_id in RETAINED_DIVISIONS:
        ...
        for f in sorted(d.glob("*.md")):
            try:
                ...
                # 3) 倒排索引：提取中文关键词
                keywords = _extract_keywords(" ".join((name_cn, desc_cn, vibe_cn, raw_name, raw_desc, raw_vibe)))
                for kw in keywords:
...
def _localized_division(item: dict, locale: str | None) -> dict:
    localized = {key: value for key, value in item.items() if not key.endswith("_en")}
    if _is_english(locale):
        localized["label"] = item.get("label_en", item["label"])
    return localized


def _localized_agent(item: dict, locale: str | None) -> dict:
    localized = {key: value for key, value in item.items() if not key.endswith("_en")}
    if _is_english(locale):
        localized["name"] = item.get("name_en", item["name"])
        localized["description"] = item.get("description_en", item["description"])
        localized["vibe"] = item.get("vibe_en", item["vibe"])
    return localized


def _cache_active_expert(locale: str | None = None) -> dict | None:
    """Return the active expert localized for the requested UI locale."""
    ...
    for a in _cache_agents:
        if a["slug"] == slug:
            localized = _localized_agent(a, locale)
            division = _cache_division_set.get(a["division"], {})
            return {
                "slug": slug,
                "name": localized["name"],
                ...
                "emoji": a["emoji"],
                "division": a["division"],
                "division_label": _localized_division(division, locale).get("label", a["division"]),
    ...
    return {
        ...
        "division_label": (
            div_meta.get("label_en", div_meta.get("label", expert.get("division", "")))
            if _is_english(locale)
            else _DIVISION_CN.get(expert.get("division", ""), div_meta.get("label", expert.get("division", "")))
        ),
        "prompt": expert.get("prompt", ""),
    }
...
async def list_divisions(locale: Optional[str] = None):
    """返回 11 个保留 Division 及其专家数量（内存缓存）。"""
    return [Division(**_localized_division(d, locale)) for d in _cache_divisions_list()]


@router.get("/agents", response_model=list[AgentSummary])
async def list_agents(division: Optional[str] = None, search: Optional[str] = None, locale: Optional[str] = None):
    """列出/搜索专家。倒排索引 O(1) 关键词查找。"""
    items = _cache_query(division, search)
    return [AgentSummary(**_localized_agent(it, locale)) for it in items]


@router.get("/market-init")
async def market_init(locale: Optional[str] = None):
    """一次性返回 divisions + agents + active expert（减少 3 次 HTTP 往返）。"""
    _build_cache()
    return {
        "divisions": [Division(**_localized_division(d, locale)) for d in _cache_divisions],
        "agents": [AgentSummary(**_localized_agent(a, locale)) for a in _cache_agents],
        "active": _cache_active_expert(locale),
    }


@router.get("/agents/{slug}", response_model=AgentDetail)
async def get_agent_detail(slug: str, locale: Optional[str] = None):
    ...
    try:
        ...
        return AgentDetail(
            slug=slug,
            name=meta.get("name", slug) if _is_english(locale) else _zh_name(meta.get("name", slug)),
            description=meta.get("description", "") if _is_english(locale) else _get_desc(meta.get("name", slug), meta.get("description", "")),
            emoji=meta.get("emoji", "🤖"),
            color=meta.get("color", "#6366F1"),
            vibe=meta.get("vibe", "") if _is_english(locale) else (_zh_desc(meta.get("vibe", "")) if meta.get("vibe") else ""),
            division=division,
            division_label=div_meta.get("label", division) if _is_english(locale) else _DIVISION_CN.get(division, div_meta.get("label", division)),
            prompt=body,
        )
...
async def get_active(locale: Optional[str] = None):
    """返回当前激活的专家（内存缓存，避免读磁盘）。"""
    cached = _cache_active_expert(locale)
    if not cached:
        return None
...
def _localized_conversation(conversation: dict, locale: str | None) -> dict:
    """Keep stored conversations locale-neutral while presenting expert labels in the UI language."""
    localized = dict(conversation)
    _build_cache()
    expert = next((item for item in _cache_agents if item["slug"] == conversation.get("expert_slug")), None)
    if not expert:
        return localized
    name = _localized_agent(expert, locale)["name"]
    if conversation.get("title") == conversation.get("expert_name"):
        localized["title"] = name
    localized["expert_name"] = name
    return localized


@router.get("/conversations", response_model=list[AgencyConversation])
async def list_agency_conversations(locale: Optional[str] = None):
    """列出 marketplace 所有历史对话（含专家信息）。"""
    return [AgencyConversation(**_localized_conversation(c, locale)) for c in _list_convs()]


@router.post("/conversations", response_model=AgencyConversation)
async def create_agency_conversation(locale: Optional[str] = None):
    """基于当前激活专家创建新对话。"""
    expert = agency_state.get_active_expert()
    if not expert:
    ...
    return AgencyConversation(**_localized_conversation(conv, locale))


@router.get("/conversations/{conv_id}", response_model=AgencyConversation)
async def get_agency_conversation(conv_id: str, locale: Optional[str] = None):
    """获取单个对话详情（含消息列表）。"""
    c = _load_conv(conv_id)
    if not c:
        raise HTTPException(404, "对话不存在")
    return AgencyConversation(**_localized_conversation(c, locale))
