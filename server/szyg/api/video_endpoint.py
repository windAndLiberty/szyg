...
async def create_video(req: CreateRequest):
    ...
    try:
        ...
        references = resolve_references(req.reference_ids, "video")
    ...
    try:
        ...
        for item in references:
            if not hasattr(client, "upload_reference"):
                raise HTTPException(503, "当前视频服务暂不支持本地参考素材上传")
            provider_url = await client.upload_reference(str(item["path"]))
            reference_assets.append({
                "type": f"{item['kind']}_url",
                "url": provider_url,
                "role": f"reference_{item['kind']}",
