...
async def generate_image(req: GenerateRequest):
    ...
    try:
        from szyg.content_reference_service import resolve_references
        references = resolve_references(req.reference_ids, "image")
    ...
    if cloud_auth.config["enabled"]:
        ...
        try:
            reference_images = [
                await client.upload_reference(str(item["path"]))
    ...
    try:
        ...
        if api_key and ep:
            ...
            reference_images = [
                "data:"
                + (str(item.get("content_type") or mimetypes.guess_type(str(item["path"]))[0] or "image/jpeg"))
                + ";base64,"
                + base64.b64encode(Path(str(item["path"])).read_bytes()).decode("ascii")
                for item in references
            ]
            paths = []
            for _ in range(req.count):
    ...
    if references:
        raise HTTPException(503, "当前图片服务暂不支持参考图生成")
