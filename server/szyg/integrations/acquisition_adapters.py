...
class BaseAcquisitionAdapter:
    ...
    async def send_comment(self, video_url: str, comment_text: str, account_id: str = "") -> dict:
    ...
    async def get_comment_replies(self, video_url: str, comment_id: str, limit: int = 30) -> list[dict]:
        """Return only replies attributable to one outbound comment."""
        rows = await self.get_comments(video_url, limit=max(limit, 30))
        target = str(comment_id or "")
        return [
            row for row in rows
            if str(row.get("root_comment_id") or "") == target
            or str(row.get("parent_comment_id") or "") == target
        ][:limit]
...
class BilibiliAcquisitionAdapter(BaseAcquisitionAdapter):
    ...
    def _load_credential_values(self, account_id: str = "") -> tuple[dict, str]:
        ...
        account = (get_account(account_id) if account_id else None) or get_default_account_for_platform("bilibili") or {}
    ...
    def _get_credential(self, account_id: str = ""):
        """Load Bilibili credentials from the shared channel-account session."""
        if self._cred_ready and self._credential_account_id == account_id:
            return self._credential

        try:
            from bilibili_api import Credential
            data, source = self._load_credential_values(account_id)
            if data:
        ...
        self._credential_account_id = account_id
        return self._credential
    ...
    async def get_comment_replies(self, video_url: str, comment_id: str, limit: int = 30) -> list[dict]:
        """Read the reply thread rooted at our verified Bilibili comment."""
        from urllib.parse import quote
        from urllib.request import Request, urlopen

        bvid = self._extract_bvid(video_url)
        if not bvid or not str(comment_id or "").isdigit():
            return []
        ...
        try:
            detail = await asyncio.to_thread(_fetch, f"https://api.bilibili.com/x/web-interface/view?bvid={quote(bvid)}")
            aid = (detail.get("data") or {}).get("aid") if detail.get("code") == 0 else None
            if not aid:
                return []
            size = max(1, min(int(limit or 30), 20))
            payload = await asyncio.to_thread(
                _fetch,
                f"https://api.bilibili.com/x/v2/reply/reply?type=1&oid={aid}&root={quote(str(comment_id))}&pn=1&ps={size}",
            )
            replies = (payload.get("data") or {}).get("replies") or []
            return [_normalize_comment({
                "comment_id": row.get("rpid", ""),
                "author": (row.get("member") or {}).get("uname", ""),
                "author_id": (row.get("member") or {}).get("mid", row.get("mid", "")),
                "text": (row.get("content") or {}).get("message", ""),
                "likes": row.get("like", 0),
                "created_at": row.get("ctime", 0),
                "root_comment_id": row.get("root", comment_id),
                "parent_comment_id": row.get("parent", comment_id),
            }, "bilibili") for row in replies[:limit]]
        except Exception as exc:
            logger.warning("Bilibili reply thread read failed: %s", exc)
            return []
    ...
    async def send_comment(self, video_url: str, comment_text: str, account_id: str = "") -> dict:
        ...
        try:
            ...
            cred = self._get_credential(account_id)
...
class PlaywrightAcquisitionAdapter(BaseAcquisitionAdapter):
    ...
    def _load_account_storage_state(self, account_id: str = "") -> dict | None:
        """Load the connected account session without mutating its source file."""
        try:
            from pathlib import Path
            from szyg.channel_accounts import get_account, get_default_account_for_platform

            account = (get_account(account_id) if account_id else None) or get_default_account_for_platform(self.platform)
    ...
    async def _ensure_browser(self, account_id: str = ""):
        ...
        storage_state = self._load_account_storage_state(account_id) or self._session.load(p)
    ...
    async def get_comments(self, video_url: str, limit: int = 30) -> list[dict]:
        ...
        async def _on_response(response):
            ...
            relevant = (
                ...
                self.platform == "xhs" and "comment" in url and any(token in url for token in ("/page", "/list", "/sub"))
            )
            if not relevant:
                return
            try:
                ...
                if self.platform == "douyin":
                    ...
                    api_comments.extend(self._normalize_xhs_api_comments(payload, limit))
            except Exception:
                return
    ...
    def _normalize_douyin_api_comments(raw_comments, limit: int = 30) -> list[dict]:
        rows: list[dict] = []
        def append_item(item: dict, root_id: str = "") -> None:
            ...
            rows.append(_normalize_comment({
                ...
                "root_comment_id": item.get("reply_to_reply_id") or item.get("root_comment_id") or root_id,
                "parent_comment_id": item.get("reply_id") or item.get("parent_comment_id") or root_id,
            }, "douyin"))
            for reply in (item.get("reply_comment") or item.get("reply_comments") or [])[:limit]:
                if isinstance(reply, dict):
                    append_item(reply, root_id or comment_id)
    ...
    def _normalize_kuaishou_api_comments(payload: dict, limit: int = 30) -> list[dict]:
        ...
        def append_item(item: dict, root_id: str = "") -> None:
            ...
            rows.append(_normalize_comment({
                ...
                "root_comment_id": item.get("rootCommentId") or item.get("root_comment_id") or root_id,
                "parent_comment_id": item.get("parentCommentId") or item.get("parent_comment_id") or root_id,
            }, "kuaishou"))
            for reply in (item.get("subComments") or item.get("sub_comments") or [])[:limit]:
                if isinstance(reply, dict):
                    append_item(reply, root_id or comment_id)
    ...
    @staticmethod
    def _normalize_xhs_api_comments(payload: dict, limit: int = 30) -> list[dict]:
        data = payload.get("data") if isinstance(payload.get("data"), dict) else payload
        raw = (data or {}).get("comments") or (data or {}).get("comment_list") or []
        rows: list[dict] = []
        ...
        def append_item(item: dict, root_id: str = "") -> None:
            ...
            rows.append(_normalize_comment({
                ...
                "text": item.get("content") or item.get("text") or "",
                "likes": item.get("like_count") or item.get("likeCount") or 0,
                "created_at": item.get("create_time") or item.get("createTime") or "",
                "root_comment_id": item.get("root_comment_id") or item.get("rootCommentId") or root_id,
                "parent_comment_id": item.get("parent_comment_id") or item.get("parentCommentId") or root_id,
            }, "xhs"))
            for reply in (item.get("sub_comments") or item.get("subComments") or item.get("replies") or [])[:limit]:
                if isinstance(reply, dict):
                    append_item(reply, root_id or comment_id)
    ...
    async def send_comment(self, video_url: str, comment_text: str, account_id: str = "") -> dict:
        """在视频页面发送评论 — 通过浏览器操作"""
        page = await self._ensure_browser(account_id)
        ...
        async def _on_response(response):
            ...
            if self._response_indicates_comment_success(payload):
                response_evidence.update({
                    ...
                    "url": response.url,
                    "comment_id": self._extract_response_comment_id(payload),
                })
            ...
            for _ in range(8):
                ...
                if response_evidence:
                    return {
                        "success": True,
                        "message": "评论已发送",
                        "comment_id": str(response_evidence.get("comment_id") or ""),
                        "source_url": page.url,
    ...
    @staticmethod
    def _extract_response_comment_id(payload: dict) -> str:
        queue = [payload]
        while queue:
            item = queue.pop(0)
            ...
            if not isinstance(item, dict):
                continue
            for key in ("comment_id", "commentId", "cid", "rpid"):
                value = item.get(key)
