...
class ComfyUIClient:
    ...
    def __init__(
        ...
        checkpoint: str = "sd21.safetensors",
        timeout: float = 300.0,
    ):
    ...
    def _copy_to_output(self, filename: str) -> Path:
        ...
        # Search in order: env var > actual nested path > D:\ComfyUI > C:\szyg
        candidates = [
            os.environ.get("COMFYUI_OUTPUT_DIR", ""),
            "D:\\szyg\\ComfyUI\\ComfyUI\\output",
            "D:\\ComfyUI\\output",
            "D:\\ComfyUI\\ComfyUI\\output",
            "C:\\szyg\\ComfyUI\\output",
            "C:\\szyg\\ComfyUI\\ComfyUI\\output",
        ]
        dst = self.output_dir / filename
        for candidate_dir in candidates:
            if not candidate_dir:
                continue
            src = Path(candidate_dir) / filename
            if src.exists():
                dst.write_bytes(src.read_bytes())
                return dst
        # Last resort: check ComfyUI's own output via API
        return dst
