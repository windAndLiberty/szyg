...
def resolve_account_session_path(account: dict) -> Path:
    """Resolve stale absolute paths after the application data directory moves."""
    configured_text = str(account.get("session_path") or "").strip()
    ...
    candidates = []
    if account.get("is_default") or account_id == f"{platform}_default":
        candidates.append(legacy_session_path(platform))
    if account_id:
        candidates.append(account_session_path(platform, account_id))
    for candidate in candidates:
        if candidate.exists():
            return candidate
    return candidates[0] if candidates else Path(configured_text)
...
def enrich_account(account: dict) -> dict:
    ...
    item["session_path"] = str(resolve_account_session_path(item))
