...
CORE_SKILLS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "action_id": "geo.audit",
        "label": "检查AI是否推荐企业",
        "description": "使用已配置的真实AI平台检查品牌提及、推荐和引用情况，并保存可复查的观测记录。",
        "risk_level": "low",
        "business_objects": ["geo", "brand", "citations"],
        "input_schema": {"provider_ids": "array", "questions": "array"},
        "output_schema": {"audit": "object", "overview": "object"},
        "execution_mode": "builtin",
    },
    {
...
class HermesWorkflowSkills:
    ...
    async def execute(self, action_id: str, params: dict[str, Any], context: dict[str, Any]) -> dict[str, Any]:
        ...
        if action_id == "geo.audit":
            from szyg.geo_service import get_geo_service

            service = get_geo_service()
            previous = next((
                item for item in service.repository.list_audits(limit=20)
                if item.get("status") in {"completed", "partial"} and item.get("mode") != "legacy"
            ), None)
            previous_rows = service.list_observations(audit_id=previous["id"], limit=5000) if previous else []
            before = service._verification_metrics(previous_rows)
            audit = service.create_audit(
                list(params.get("provider_ids") or []) or None,
                list(params.get("questions") or []) or None,
                question_ids=list(params.get("question_ids") or []) or None,
                sample_count=1,
                mode="scheduled",
            )
            audit = await service.run_audit(audit["id"])
            if audit.get("status") == "queued":
                return {
                    "audit": audit,
                    "overview": service.overview(30),
                    "before": before,
                    "after": before,
                    "changes": {key: 0.0 for key in ("mention_rate", "recommendation_rate", "owned_citation_rate", "accuracy_rate")},
                    "alert_required": False,
                    "alert_reasons": [],
                    "message": "本周检测已准备好；打开GEO品牌体检后，超级员工会在右侧真实AI界面中完成询问",
                }
            current_rows = service.list_observations(audit_id=audit["id"], limit=5000)
            after = service._verification_metrics(current_rows)
            changes = {
            ...
            current_failed = {row.get("provider_id") for row in current_rows if row.get("status") == "failed"}
            previous_failed = {row.get("provider_id") for row in previous_rows if row.get("status") == "failed"}
            consecutive_failures = sorted(str(item) for item in current_failed & previous_failed if item)
            factual_issue_count = sum(len(row.get("factual_issues") or []) for row in current_rows)
            alert_reasons = []
            if previous and changes["recommendation_rate"] <= -10:
                alert_reasons.append(f"推荐率较上次下降 {abs(changes['recommendation_rate'])} 个百分点")
            if factual_issue_count:
                alert_reasons.append(f"发现 {factual_issue_count} 条品牌事实错误")
            if consecutive_failures:
                alert_reasons.append(f"平台连续检测失败：{'、'.join(consecutive_failures)}")
            return {
                "audit": audit, "overview": service.overview(30), "before": before, "after": after,
                "changes": changes, "alert_required": bool(alert_reasons), "alert_reasons": alert_reasons,
                "message": "；".join(alert_reasons) if alert_reasons else "本周检测完成，关键指标没有明显下降",
            }
