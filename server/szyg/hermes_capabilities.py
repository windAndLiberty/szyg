...
def _geo_audit_body(args: dict[str, Any]) -> dict[str, Any]:
    return {
        "provider_ids": _string_list(args.get("provider_ids")),
        "questions": _string_list(args.get("questions")),
        "question_ids": _string_list(args.get("question_ids")),
        "sample_count": int(args.get("sample_count") or 1),
        "mode": str(args.get("mode") or "scheduled"),
    }
...
def _capabilities() -> list[Capability]:
    ...
    return [
        ...
        Capability("geo.profile", "geo", "GEO 企业资料", "读取用于 AI 品牌监测的官网、品牌别名、产品、竞品、地区语言和客户问题。", path="/api/geo/profile"),
        Capability("geo.overview", "geo", "GEO 品牌概览", "查看品牌在已接入 AI 平台中的提及率、推荐率、引用次数、错误信息和竞品差距。", path="/api/geo/overview", query_keys=("days",), parameters={"days": {"type": "integer", "default": 30}}),
        Capability("geo.observations", "geo", "GEO 监测证据", "读取各客户问题在 AI 平台中的真实回答、品牌提及、推荐和引用证据。", path="/api/geo/observations", query_keys=("audit_id", "provider_id", "limit"), parameters={"audit_id": scalar, "provider_id": scalar, "limit": {"type": "integer", "default": 100}}),
        Capability("geo.recommendations", "geo", "GEO 优化建议", "读取按不了解、未推荐、错误信息和竞品领先组织的品牌优化建议。", path="/api/geo/recommendations", query_keys=("limit",), parameters={"limit": {"type": "integer", "default": 100}}),
        Capability("geo.audit", "geo", "执行 GEO 品牌检查", "向已真实接入的 AI 搜索平台提交客户问题，异步检查品牌是否被提及、推荐和引用；未接入平台不会生成模拟结果。", method="POST", path="/api/geo/audits", risk="analysis", body_builder=_geo_audit_body, timeout=30, parameters={"provider_ids": {"type": "string", "description": "可选平台 ID，逗号分隔或 JSON 数组"}, "questions": {"type": "string", "description": "可选客户问题，逗号分隔或 JSON 数组"}, "question_ids": {"type": "string"}, "sample_count": {"type": "integer", "default": 1}, "mode": scalar}),
        Capability("geo.questions.suggest", "geo", "发现客户会问的问题", "结合企业资料和营销情报生成可确认的客户问题，不会自动开始检测。", method="POST", path="/api/geo/questions/suggest", risk="analysis", body_builder=lambda args: {"limit": int(args.get("limit") or 12)}, parameters={"limit": {"type": "integer", "default": 12}}),
        Capability("geo.sources.analyze", "geo", "分析AI引用来源", "查看最常被AI引用的来源，以及支持竞品但尚未支持本品牌的来源机会。", path="/api/geo/sources", query_keys=("limit",), parameters={"limit": {"type": "integer", "default": 100}}),
        Capability("geo.recommendation.get", "geo", "读取GEO改进任务", "读取一条GEO改进建议及其回答、引用和竞品证据。", path_builder=lambda args: f"/api/geo/recommendations/{quote(_safe_id(args.get('recommendation_id'), '建议 ID'))}", parameters={"recommendation_id": scalar}),
        Capability("geo.recommendation.verify", "geo", "复查GEO改进结果", "针对一条已完成的GEO建议，以三次采样重新检查受影响问题。", method="POST", risk="analysis", path_builder=lambda args: f"/api/geo/recommendations/{quote(_safe_id(args.get('recommendation_id'), '建议 ID'))}/verify", parameters={"recommendation_id": scalar}),
        Capability("geo.site.review", "geo", "检查官网AI可发现性", "检查官网公开访问、标题、结构化数据、robots.txt和网站地图，不修改官网。", method="POST", path="/api/geo/site-review", risk="analysis"),
...
CAPABILITY_TOOL_SCHEMAS = [
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {
                    "domain": {"type": "string", "enum": ["content", "knowledge", "private_domain", "intelligence", "geo", "acquisition", "customers", "insights", "materials", "publishing", "workflows", "execution"]},
