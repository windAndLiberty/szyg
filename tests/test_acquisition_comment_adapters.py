...
def test_nested_platform_replies_keep_thread_attribution():
    rows = PlaywrightAcquisitionAdapter._normalize_douyin_api_comments([{
        "cid": "root-1",
        "text": "主评论",
        "user": {"nickname": "经营号", "uid": "owner-1"},
        "reply_comment": [{
            "cid": "reply-1",
            "text": "多少钱，怎么联系？",
            "user": {"nickname": "潜在客户", "uid": "lead-1"},
        }],
    }])

    assert rows[1]["root_comment_id"] == "root-1"
    assert rows[1]["parent_comment_id"] == "root-1"
    assert rows[1]["author_id"] == "lead-1"


def test_platform_response_comment_id_is_extracted():
    assert PlaywrightAcquisitionAdapter._extract_response_comment_id({
        "status_code": 0,
        "data": {"comment": {"comment_id": "comment-42"}},
    }) == "comment-42"
