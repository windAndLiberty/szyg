  ...
  const handleFile = async (file?: File | null) => {
    ...
    try {
      ...
      if (!setAvatarCustom(dataUrl)) {
        onError?.('图片过大，本机存储空间不足，请换一张小一点的图片')
      }
    } catch {
      onError?.('图片处理失败，请换一张图片重试')
    } finally {
      setSaving(false)
        ...
        >
          ...
          >
            <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-[#1E293B]">
              <h3 className="text-base font-semibold text-[#F1F5F9]">设置头像</h3>
              <span className="text-xs text-[#64748B]">仅保存在本机，不会上传云端</span>
            </div>
            ...
            <div className="px-5 py-5 space-y-6">
              ...
              {/* 内置头像选择 */}
              <div>
                <p className="mb-3 text-sm text-[#94A3B8]">内置头像</p>
                <div className="grid grid-cols-5 gap-3">
                  {AVATAR_PRESETS.map((src, index) => (
                    <button
                      ...
                      title={index === 0 ? '默认头像' : `内置头像 ${index + 1}`}
                    >
                      <img src={src} alt={index === 0 ? '默认头像' : `内置头像 ${index + 1}`} loading="lazy" draggable={false} className="h-full w-full object-cover" />
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="flex items-center justify-between gap-3 rounded-xl border border-dashed border-[#334155] bg-[#0D1321]/60 px-4 py-3">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-[#F1F5F9]">上传自定义头像</p>
                    <p className="mt-0.5 text-xs text-[#64748B]">支持 JPG / PNG，将自动压缩后保存在本机</p>
                  </div>
                </div>
