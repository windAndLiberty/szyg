...
export default function CloudAuthExpiredBanner() {
  ...
  const show = useCallback((detail?: string) => {
    setMessage(detail || '服务授权已到期，智能服务暂不可用，请重新登录或联系管理员续期')
    setVisible(true)
  }, [])
  ...
  const handleRefresh = useCallback(async () => {
    ...
    try {
      ...
      if (!result.expired) {
        ...
        setError(result.authorization_error || '服务授权仍然到期，请联系管理员续期后再刷新')
      }
    } catch (err) {
      setError(toUserFacingMessage(err as Error, '刷新授权状态失败，请稍后重试'))
    } finally {
      setRefreshing(false)
    }
  }, [refreshing, hide])
    ...
    <>
          ...
          <div className="mx-auto flex max-w-5xl flex-col gap-3 px-4 py-3 sm:flex-row sm:items-center sm:gap-4 sm:px-6">
            <div className="flex flex-1 items-start gap-3 text-[#FCD34D] sm:items-center">
              ...
              <div className="min-w-0">
                <p className="text-sm font-medium text-[#FDE68A]">服务授权已到期</p>
      ...
      {loginOpen && (
        <div className="fixed inset-0 z-[160] bg-[#0B0F1A]">
          <CloudLogin
            initialMessage="服务授权已到期，请重新登录以恢复智能服务"
            onSuccess={async () => {
