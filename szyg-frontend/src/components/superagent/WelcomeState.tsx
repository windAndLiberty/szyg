...
const skillShortcuts = [
  {
    get label() { return translateCurrent("代办网页工作") },
    get prompt() { return translateCurrent("请告诉我你希望访问的网站和要完成的事情；需要登录、提交或外发时先让我确认。") },
    icon: Globe2,
  },
  {
    get label() { return translateCurrent("策划营销内容") },
    get prompt() { return translateCurrent("请结合我的企业资料，策划一套适合当前产品的营销内容，并说明推荐平台和执行步骤。") },
    icon: Film,
  },
  {
    get label() { return translateCurrent("发现潜在客户") },
    get prompt() { return translateCurrent("请结合我的产品与知识库，寻找高相关的潜在客户机会，并给出安全的触达建议。") },
    icon: Target,
  },
  {
    get label() { return translateCurrent("推进客户转化") },
    get prompt() { return translateCurrent("请分析当前高意向线索，生成个性化跟进建议和下一步行动。") },
    icon: UsersRound,
  },
  {
    get label() { return translateCurrent("复盘运营表现") },
    get prompt() { return translateCurrent("请汇总最近的内容、发布、获客和转化数据，找出最值得优先处理的问题。") },
    icon: BarChart3,
  },
]
    ...
    >
      ...
      >
        ...
        <div className="text-center">
          <p className="text-body-md text-[#94A3B8]">{t("一个超级员工，按需组合内容、获客、转化与运营技能")}</p>
          <div className="mt-3 flex flex-wrap items-center justify-center gap-2 text-[11px] text-[#64748B]">
            ...
            {memoryEnabled && (
              <span className="rounded-full border border-[#334155] bg-[#111827]/70 px-2.5 py-1">{t("长期记忆已开启")}</span>
            )}
          </div>
        </div>
      </motion.div>
      ...
      >
        <div className="flex items-center gap-2 mb-4">
          ...
          <span className="text-heading-sm text-[#F1F5F9]">{t("智能员工 · 精选案例")}</span>
        </div>
        ...
        <div className="h-[132px] overflow-x-auto overflow-y-hidden">
          {caseCardsLoading ? (
            <div className="flex h-full items-center justify-center text-[#64748B] gap-2">
              ...
              <span className="text-[13px]">{t("正在搜索真实推荐案例…")}</span>
            </div>
          ) : caseCards.length === 0 ? (
            <div className="flex h-full flex-col items-center justify-center text-[#64748B] gap-1">
              <span className="text-[13px]">{t("暂无推荐案例")}</span>
              <span className="text-[11px]">{t("发送一条消息即可开始对话")}</span>
            </div>
          ) : (
                ...
                >
                  ...
                  >
                    <div className="flex w-full items-center justify-between gap-2 text-[11px]">
                      <span className="inline-flex items-center gap-1.5 font-medium text-[#818CF8]">
                        ...
                        {sourceLabel[card.source] || t("视频案例")}
                      </span>
                  ...
                  {card.video_url && (
                    <a
                      ...
                      aria-label={t("打开{v0}", { v0: card.title })}
                      title={t("打开原视频")}
                    >
