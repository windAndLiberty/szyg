    ...
    <aside className="relative flex min-h-0 shrink-0 flex-col border-l border-[#243047] bg-[#0B1020]" style={{ width }}>
      ...
      <header className="flex h-14 shrink-0 items-center gap-3 border-b border-[#243047] px-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="truncate text-sm font-medium text-[#F4F7FF]">{taskTitle || '工作现场'}</p>
        </div>
        {paused || browserState.owner === 'user' ? (
          <button onClick={handleResume} className="inline-flex h-8 items-center gap-1.5 px-2.5 text-xs text-[#C7D2FE] hover:bg-[#1E293B]" title="继续">
            <Play className="h-3.5 w-3.5" />继续
          </button>
        ) : (
          <button onClick={handleTakeover} className="inline-flex h-8 items-center gap-1.5 px-2.5 text-xs text-[#CBD5E1] hover:bg-[#1E293B]" title="暂时由我操作">
            <Hand className="h-3.5 w-3.5" />由我操作
          </button>
        )}
        {(busy || phase === 'running' || phase === 'paused') && (
          <button onClick={handleStop} className="grid h-8 w-8 place-items-center text-[#94A3B8] hover:bg-[#1E293B] hover:text-[#FCA5A5]" title="停止">
            <Square className="h-3.5 w-3.5" />
          </button>
        )}
        <button onClick={toggleExpanded} className="grid h-8 w-8 place-items-center text-[#94A3B8] hover:bg-[#1E293B] hover:text-white" title={expanded ? '恢复宽度' : '展开'}>
          {expanded ? <Minimize2 className="h-4 w-4" /> : <Maximize2 className="h-4 w-4" />}
        </button>
        <button onClick={onClose} className="grid h-8 w-8 place-items-center text-[#94A3B8] hover:bg-[#1E293B] hover:text-white" title="关闭">
          <X className="h-4 w-4" />
        </button>
      </header>

      <div className="flex h-11 shrink-0 items-center gap-1 border-b border-[#243047] bg-[#0E1525] px-2">
        <button disabled={!browserState.canGoBack} onClick={() => void browserAction('back')} className="grid h-8 w-8 place-items-center text-[#94A3B8] hover:bg-[#1E293B] disabled:opacity-30" title="后退">
          <ArrowLeft className="h-4 w-4" />
        </button>
        <button disabled={!browserState.canGoForward} onClick={() => void browserAction('forward')} className="grid h-8 w-8 place-items-center text-[#94A3B8] hover:bg-[#1E293B] disabled:opacity-30" title="前进">
          <ArrowRight className="h-4 w-4" />
        </button>
        <button onClick={() => void browserAction('refresh')} className="grid h-8 w-8 place-items-center text-[#94A3B8] hover:bg-[#1E293B]" title="刷新">
        ...
        <div className="mx-1 flex min-w-0 flex-1 items-center gap-2 rounded-md border border-[#26334A] bg-[#111827] px-3 py-1.5">
          ...
          <span className="truncate text-xs text-[#CBD5E1]">{hostname(browserState.url) || '等待打开网页'}</span>
        </div>
        {browserState.owner === 'user' && <span className="px-2 text-[11px] text-[#FCD34D]">你正在操作</span>}
      </div>
        ...
        {!hasDesktopBrowser && (
          <div className="absolute inset-0 grid place-items-center px-10 text-center">
            <div>
              ...
              <p className="text-sm text-[#CBD5E1]">网页处理画面将在桌面版中实时显示</p>
              <p className="mt-1 text-xs text-[#64748B]">当前开发网页仅显示任务进度与确认信息</p>
            </div>
          </div>
        )}
      </div>
          ...
          <div className="flex items-start gap-2">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-3 flex gap-2">
                <button onClick={() => onApproval('approve_once')} className="h-8 bg-[#F4F7FF] px-3 text-xs font-medium text-[#111827] hover:bg-white">确认继续</button>
                <button onClick={() => onApproval('deny')} className="h-8 border border-[#5B4A24] px-3 text-xs text-[#D6C48A] hover:bg-[#2A220E]">取消</button>
              </div>
            </div>
          </div>
        </section>
        ...
        <div className="flex min-h-12 items-center gap-2 px-4">
          ...
          {visibleEvents.length > 0 && (
            <button onClick={() => setDetailsOpen((value) => !value)} className="inline-flex items-center gap-1 text-xs text-[#94A3B8] hover:text-white">
              {detailsOpen ? '收起记录' : `查看记录 ${visibleEvents.length}`}
