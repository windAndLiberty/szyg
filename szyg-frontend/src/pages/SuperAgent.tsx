...
const COMMON_SKILLS: ComposerSkill[] = [
  {
    id: 'content-planning',
    get name() { return translateCurrent("内容策划") },
    emoji: '✍️',
    get description() { return translateCurrent("策划营销选题、文案和多平台内容方案。") },
    guidance: '优先使用内容策划、知识库和内容生成能力，给出可直接执行的营销内容方案。',
  },
  {
    id: 'lead-discovery',
    get name() { return translateCurrent("客户发现") },
    emoji: '🎯',
    get description() { return translateCurrent("发现高相关潜在客户并规划安全触达方式。") },
    guidance: '优先使用公域搜索、客户线索和企业知识能力，寻找高相关潜在客户机会。',
  },
  {
    id: 'sales-followup',
    get name() { return translateCurrent("转化跟进") },
    emoji: '🤝',
    get description() { return translateCurrent("判断客户意向并准备个性化跟进建议。") },
    guidance: '优先使用客户、线索和转化分析能力，形成个性化跟进建议与下一步行动。',
  },
  {
    id: 'operations-review',
    get name() { return translateCurrent("运营复盘") },
    emoji: '📊',
    get description() { return translateCurrent("汇总经营数据，识别问题并给出优化动作。") },
...
function workToolPresentation(tool: string, rawArgs: unknown): Omit<BrowserWorkEvent, 'id' | 'state'> | null {
  ...
  if (tool === 'szyg_browser') {
    ...
    const mapping: Record<string, string> = {
      get open() { return translateCurrent("打开网页") },
      get navigate() { return translateCurrent("打开网页") },
      get observe() { return translateCurrent("理解当前页面") },
      get read() { return translateCurrent("读取网页结果") },
      get click() { return translateCurrent("操作页面内容") },
      get type() { return translateCurrent("填写网页内容") },
      get press() { return translateCurrent("提交网页内容") },
      get scroll() { return translateCurrent("浏览页面") },
      get back() { return translateCurrent("返回上一页") },
      get forward() { return translateCurrent("前往下一页") },
      get refresh() { return translateCurrent("刷新网页") },
    }
    return { type: 'browser', title: mapping[action] || translateCurrent("操作网页"), technicalName: `${tool}:${action || 'action'}` }
  }
  if (tool === 'terminal') return { type: 'system', get title() { return translateCurrent("准备运行环境") }, technicalName: tool }
  if (/publish|upload/i.test(tool)) return { type: 'business', get title() { return translateCurrent("提交发布任务") }, technicalName: tool }
  if (/search|intelligence|browser/i.test(tool)) return { type: 'business', get title() { return translateCurrent("查找相关信息") }, technicalName: tool }
  if (/knowledge|memory|document|file/i.test(tool)) return { type: 'business', get title() { return translateCurrent("整理所需资料") }, technicalName: tool }
  if (/workflow|scheduler|task/i.test(tool)) return { type: 'business', get title() { return translateCurrent("执行工作流程") }, technicalName: tool }
  if (/lead|customer|comment|acquisition/i.test(tool)) return { type: 'business', get title() { return translateCurrent("处理客户任务") }, technicalName: tool }
  if (/image|video|audio|tts|content|copy/i.test(tool)) return { type: 'business', get title() { return translateCurrent("生成内容") }, technicalName: tool }
  return { type: 'business', get title() { return translateCurrent("执行任务步骤") }, technicalName: tool }
}
  ...
  const loadComposerSkills = useCallback(async () => {
    try {
      ...
      const installed = (items || []).map((item) => ({
          ...
          name: item.name,
          emoji: '🧩',
          description: item.description || t("已添加到超级员工，可在合适的工作中使用。"),
...
const saveConversation = useCallback(async (messagesSnapshot?: ChatMessage[]) => {
    try {
      ...
      const payload = {
        title: firstUser ? firstUser.content.slice(0, 50) : t("新对话"),
  ...
  const handleEvent = useCallback(
    (ev: HermesEvent) => {
      switch (ev.type) {
        ...
        case 'tool.started': {
          const name = String(ev.tool || t("正在执行"))
        ...
        case 'tool.completed': {
          const name = String(ev.tool || t("操作"))
          const presentation = workToolPresentation(name, ev.args)
          if (!presentation) break
          ...
          upsertWorkEvent({
            ...
            ...presentation,
            detail: failed ? toUserFacingMessage(rawResult, t("这一步未能完成")) : undefined,
            state: failed ? 'error' : 'success',
          })
          if (failed) setWorkStatus(t("这一步需要处理"))
          break
        }
        case 'tool.progress': {
          const progress = String(ev.content || '').trim()
          if (progress && progress.length <= 60 && !/(tool|mcp|runtime|schema|callback)/i.test(progress)) {
            setWorkStatus(toUserFacingMessage(progress, t("正在继续任务")))
          }
          break
        }
        case 'browser.action':
        ...
        case 'approval.required':
          setWorkApproval({
            ...
            title: String(ev.content || t("需要你的确认")),
            summary: String(ev.summary || ''),
          })
          setWorkViewOpen(true)
          setWorkPhase('waiting')
          setWorkStatus(t("等待你的确认"))
          break
        case 'run.started':
          ensureStreamMsg()
          setStatusText(t("正在理解任务"))
          setWorkPaused(false)
          setWorkPhase('running')
          setWorkStatus(t("正在理解任务"))
          break
        case 'run.paused':
          setWorkPaused(true)
          setWorkPhase('paused')
          setWorkStatus(String(ev.content || t("已暂停")))
          break
case 'run.completed':
          ensureStreamMsg()
...
case 'run.failed':
          ...
          replaceStreamText(toUserFacingMessage(ev.content, t("当前操作未完成")))
          setWorkEvents((current) => current.map((item) => item.state === 'running' ? { ...item, state: 'error', get detail() { return translateCurrent("任务在这一步停止") } } : item))
          setWorkPhase(String(ev.code || '') === 'cancelled' ? 'cancelled' : 'failed')
          setWorkStatus(toUserFacingMessage(ev.content, t("当前操作未完成")))
          break
        case 'error':
          ensureStreamMsg()
          appendText('\n⚠️ ' + toUserFacingMessage(ev.content, t("当前操作未完成，请稍后重试")))
          break
        case 'status':
          setStatusText(toUserFacingMessage(ev.content, t("正在处理")))
          break
        case 'status.changed':
          setStatusText(toUserFacingMessage(ev.content, t("正在处理")))
          if (!/(tool|mcp|runtime|schema|callback)/i.test(String(ev.content || ''))) {
            setWorkStatus(toUserFacingMessage(ev.content, t("正在处理")))
          }
          break
        default:
          break
  ...
  const runGeneration = useCallback(
    async (text: string) => {
      ...
      if (kind === 'image') {
        ensureStreamMsg()
        appendText(t("正在生成图片：{v0}", { v0: prompt }))
        try {
          const res = await generateImage(prompt)
          if (res.images && res.images.length > 0) {
            addMessage({ role: 'assistant', type: 'image', content: '', image_url: res.images[0], prompt })
          } else {
            appendText(t("⚠️ 图片生成未返回结果"))
          }
        } catch (e) {
          appendText(t("⚠️ 图片生成失败: {v0}", { v0: getErrorMessage(e) }))
        }
      } else {
        ensureStreamMsg()
        appendText(t("正在提交视频生成任务：{v0}", { v0: prompt }))
        let taskId = ''
        try {
          ...
          appendText(t("⚠️ 视频任务提交失败: {v0}", { v0: getErrorMessage(e) }))
          return
        }
        for (let i = 0; i < 120; i++) {
          await new Promise((r) => setTimeout(r, 4000))
          try {
            const videoTask = await pollVideoTask(taskId)
            setMessages((prev) =>
              prev.map((m) =>
                m.type === 'video_pending' && m.task_id === taskId
                  ? { ...m, status: videoTask.status, progress: videoTask.progress ?? m.progress }
                  : m,
              ),
            )
            if (videoTask.status === 'succeeded' || videoTask.status === 'succeed') {
              if (videoTask.video_url) {
                setMessages((prev) =>
                  prev.map((m) =>
                    m.type === 'video_pending' && m.task_id === taskId
                      ? { ...m, type: 'video', video_url: videoTask.video_url }
                      : m,
                  ),
                )
              }
            ...
            if (videoTask.status === 'failed' || videoTask.error) {
              setMessages((prev) =>
                prev.map((m) =>
                  m.type === 'video_pending' && m.task_id === taskId
                    ? { ...m, status: 'failed' }
              ...
              appendText(t("⚠️ 视频生成失败: {v0}", { v0: videoTask.error || videoTask.status }))
              break
            }
          } catch {
  ...
  const handleFilesSelected = useCallback(async (files: File[]) => {
    ...
    const pending = selected.map((file) => ({
      ...
      attachment: {
        ...
        error: file.size > 80 * 1024 * 1024 ? t("文件超过 80 MB") : undefined,
      },
    }))
    ...
    await Promise.allSettled(pending.map(async ({ file, attachment }) => {
      ...
      try {
        ...
        setComposerAttachments((current) => current.map((item) => item.id === attachment.id
          ? { ...item, status: 'error', error: getErrorMessage(error, t("上传失败")) }
          : item))
      }
    }))
  ...
  const sendMessage = useCallback(
    async (overrideText?: string) => {
      ...
      const text = (overrideText ?? inputText).trim() || (readyAttachments.length > 0 ? t("请阅读并分析我上传的附件。") : '')
      ...
      try {
        ...
        ensureStreamMsg()
        appendText(t("⚠️ 请求失败: {v0}", { v0: getErrorMessage(e) }))
      } finally {
        const id = streamMsgIdRef.current
  ...
  const controlRuntime = useCallback(async (action: 'takeover' | 'resume' | 'stop') => {
    ...
    try {
      ...
      if (action === 'takeover') {
        setWorkPaused(true)
        setWorkPhase('paused')
        setWorkStatus(t("已暂停，当前窗口交由你操作"))
      } else if (action === 'resume') {
        setWorkPaused(false)
        setWorkPhase('running')
        setWorkStatus(t("可以继续交代任务"))
      } else {
        setWorkPaused(false)
        setWorkPhase('cancelled')
        setWorkStatus(t("任务已停止"))
      }
    } catch (error) {
