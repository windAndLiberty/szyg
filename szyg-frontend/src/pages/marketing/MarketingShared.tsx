...
export const PLATFORM_LABELS: Record<string, string> = {
  get douyin() { return translateCurrent("抖音") },
  get xhs() { return translateCurrent("小红书") },
  get kuaishou() { return translateCurrent("快手") },
  get bilibili() { return translateCurrent("B站") },
  get tencent() { return translateCurrent("视频号") },
  get weibo() { return translateCurrent("微博") },
  youtube: 'YouTube',
  get wechat() { return translateCurrent("微信") },
  get wechat_mp() { return translateCurrent("公众号") },
}

export const PLATFORM_OPTIONS = [
  { value: 'douyin', get label() { return translateCurrent("抖音") } },
  { value: 'xhs', get label() { return translateCurrent("小红书") } },
  { value: 'kuaishou', get label() { return translateCurrent("快手") } },
  { value: 'bilibili', get label() { return translateCurrent("B站") } },
]

export const STRATEGY_OPTIONS = [
  { value: 'balanced', get label() { return translateCurrent("均衡") } },
  { value: 'friendly', get label() { return translateCurrent("亲和") } },
  { value: 'professional', get label() { return translateCurrent("专业") } },
  { value: 'cautious', get label() { return translateCurrent("谨慎") } },
]

export const FUNNEL_STAGE_LABELS: Record<string, string> = {
  get discovered() { return translateCurrent("发现线索") },
  get replied() { return translateCurrent("已回复") },
  get dm_sent() { return translateCurrent("已私信") },
  get responded() { return translateCurrent("已回应") },
  get qualified() { return translateCurrent("已确认") },
  get converted() { return translateCurrent("已转化") },
}
...
export function parseFunnelStages(data: Record<string, unknown> | undefined): FunnelStage[] {
  ...
  return raw
    .map((item) => {
      ...
      return {
        stage,
        label: FUNNEL_STAGE_LABELS[stage] || stage || translateCurrent("未知阶段"),
...
export function formatNumber(value: unknown): string {
  ...
  if (n >= 10000) return translateCurrent("{v0}万", { v0: (n / 10000).toFixed(1) })
...
export function platformLabel(platform?: string): string {
  if (!platform) return translateCurrent("未知平台")
...
export function statusTone(status?: string): { label: string; className: string; icon: LucideIcon } {
  ...
  if (['success', 'sent', 'completed', 'active', 'ok'].includes(normalized)) {
    return { get label() { return translateCurrent("成功") }, className: 'text-[#10B981] bg-[#10B981]/10 border-[#10B981]/25', icon: CheckCircle2 }
  }
  if (['failed', 'error', 'blocked', 'risk'].includes(normalized)) {
    return { get label() { return translateCurrent("失败") }, className: 'text-[#EF4444] bg-[#EF4444]/10 border-[#EF4444]/25', icon: AlertTriangle }
  }
  if (['running', 'sending', 'processing'].includes(normalized)) {
    return { get label() { return translateCurrent("执行中") }, className: 'text-[#3B82F6] bg-[#3B82F6]/10 border-[#3B82F6]/25', icon: Loader2 }
  }
  if (['auto_repaired'].includes(normalized)) {
    return { get label() { return translateCurrent("已自动修复") }, className: 'text-[#38BDF8] bg-[#38BDF8]/10 border-[#38BDF8]/25', icon: CheckCircle2 }
  }
  if (['delayed'].includes(normalized)) {
    return { get label() { return translateCurrent("延后发送") }, className: 'text-[#A78BFA] bg-[#A78BFA]/10 border-[#A78BFA]/25', icon: Clock }
  }
  if (['retrying'].includes(normalized)) {
    return { get label() { return translateCurrent("自动重试") }, className: 'text-[#3B82F6] bg-[#3B82F6]/10 border-[#3B82F6]/25', icon: Loader2 }
  }
  if (['skipped'].includes(normalized)) {
    return { get label() { return translateCurrent("已跳过") }, className: 'text-[#94A3B8] bg-[#64748B]/10 border-[#64748B]/25', icon: Clock }
  }
  if (['needs_human', 'review', 'pending_review'].includes(normalized)) {
    return { get label() { return translateCurrent("需人工") }, className: 'text-[#F59E0B] bg-[#F59E0B]/10 border-[#F59E0B]/25', icon: AlertTriangle }
  }
  if (['queued', 'pending', 'waiting'].includes(normalized)) {
    return { get label() { return translateCurrent("排队中") }, className: 'text-[#A78BFA] bg-[#A78BFA]/10 border-[#A78BFA]/25', icon: Clock }
  }
  return { label: status ? String(status) : translateCurrent("未开始"), className: 'text-[#94A3B8] bg-[#64748B]/10 border-[#64748B]/25', icon: Clock }
}
