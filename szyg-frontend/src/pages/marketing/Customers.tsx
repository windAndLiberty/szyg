...
export default function Customers() {
  ...
  const loadCustomers = async (query = search) => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : t("客户资产加载失败"))
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <PageShell title={t("客户资产池")} subtitle={t("把线索沉淀为客户资产，保留来源、互动记录和下一步动作。")} icon={Users}>
      {error && <div className="rounded-lg border border-[#EF4444]/30 bg-[#EF4444]/10 px-4 py-3 text-sm text-[#FCA5A5]">{error}</div>}

      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard label={t("客户总数")} value={customers.length} icon={Users} />
        <MetricCard label={t("高意向客户")} value={customers.filter((item) => ['a', 's', '高', 'high'].includes(String(item.grade || '').toLowerCase())).length} icon={UserRound} tone="green" />
        <MetricCard label={t("待跟进")} value={customers.filter((item) => !['done', 'converted', 'invalid'].includes(String(item.status || '').toLowerCase())).length} icon={MessageCircle} tone="amber" />
        <MetricCard label={t("标签数量")} value={new Set(customers.flatMap((item) => item.tags || [])).size} icon={Tags} tone="blue" />
      </div>

      <div className="grid gap-5 xl:grid-cols-[1fr_420px]">
        <Panel
          title={t("客户列表")}
          description={t("按客户昵称、最近消息或标签检索客户资产。")}
          action={
            <div className="flex items-center gap-2">
              <input value={search} onChange={(e) => setSearch(e.target.value)} className={`${inputClass} h-9 w-52`} placeholder={t("搜索客户")} />
              <button onClick={() => loadCustomers(search)} className={buttonSecondary}><Search className="h-4 w-4" />{t("搜索")}</button>
            </div>
          }
        >
          {loading ? (
            ...
            <EmptyState title={t("暂无客户资产")} description={t("客户会从线索跟进中沉淀，也可以后续从微信、企微、私信渠道同步。")} />
          ) : (
            <div className="overflow-hidden rounded-lg border border-[#1E293B]">
              <table className="w-full text-left text-sm">
                <thead className="bg-[#0B0F1A] text-xs text-[#64748B]">
                  <tr>
                    <th className="px-3 py-3">{t("客户")}</th>
                    <th className="px-3 py-3">{t("来源")}</th>
                    <th className="px-3 py-3">{t("意向")}</th>
                    <th className="px-3 py-3">{t("状态")}</th>
                    <th className="px-3 py-3">{t("下一步")}</th>
                  </tr>
                </thead>
                  ...
                  {customers.map((item, index) => (
                    <tr key={item.id || index} onClick={() => setSelected(item)} className={`cursor-pointer ${selected === item ? 'bg-[#6366F1]/10' : 'hover:bg-[#0B0F1A]/60'}`}>
                      <td className="px-3 py-3">
                        <p className="font-medium text-[#F1F5F9]">{item.name || t("未命名客户")}</p>
                        <p className="mt-1 truncate text-xs text-[#64748B]">{item.lastMessage || item.source || t("暂无最近互动")}</p>
                      </td>
                      ...
                      <td className="px-3 py-3 text-[#94A3B8]">{item.next_action || t("继续跟进")}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <Panel title={t("客户详情")} description={t("查看来源链路、AI 总结和互动时间线。")}>
          {!selected ? (
            <EmptyState title={t("请选择客户")} description={t("选择客户后查看消息记录、标签和推荐动作。")} />
          ) : (
            <div className="space-y-4">
              <div className="rounded-lg border border-[#1E293B] bg-[#0B0F1A] p-4">
                <p className="text-base font-semibold text-[#F1F5F9]">{selected.name || t("未命名客户")}</p>
                <p className="mt-1 text-xs text-[#64748B]">{platformLabel(selected.platform)}  {t("· 负责人")} {selected.owner || t("未分配")}</p>
              ...
              <div className="rounded-lg border border-[#1E293B] bg-[#0B0F1A] p-4">
                ...
                {loadingMessages ? (
                  ...
                  <p className="text-xs text-[#64748B]">{t("暂无消息记录")}</p>
                ) : (
