...
export default function Conversion() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : '客户转化数据加载失败')
    } finally {
      setLoading(false)
    }
  }
  ...
  const confirmDm = () => {
    if (confirm('这是私信发送的预览确认入口。请确保话术、账号状态和平台规则已经人工检查。')) {
      alert('v1 保留确认入口。真实私信发送将接入队列和执行观测。')
    }
  }
  ...
  return (
    <PageShell title="客户转化工作台" subtitle="把识别出的线索推进到回复、私信、确认和成交阶段。" icon={Repeat2}>
      {error && <div className="rounded-lg border border-[#EF4444]/30 bg-[#EF4444]/10 px-4 py-3 text-sm text-[#FCA5A5]">{error}</div>}

      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard label="线索总数" value={leads.length} icon={UserCheck} />
        <MetricCard label="待跟进" value={pending} icon={MessageCircle} tone="amber" />
        <MetricCard label="高意向" value={highIntent} icon={Sparkles} tone="green" />
        <MetricCard label="漏斗阶段" value={funnelStages.length || '-'} icon={Funnel} tone="blue" />
      </div>

      <div className="grid gap-5 xl:grid-cols-[1fr_420px]">
        <Panel title="待跟进线索" description="按最新状态处理线索，优先跟进高意向用户。">
          {loading ? (
            <div className="flex h-40 items-center justify-center text-[#64748B]"><Loader2 className="h-6 w-6 animate-spin" /></div>
          ) : leads.length === 0 ? (
            <EmptyState title="暂无线索" description="智能截流和机会监听识别到线索后，会进入这里。" />
          ) : (
            <div className="space-y-3">
              {leads.map((item, index) => (
                ...
                >
                  ...
                  <p className="mt-3 line-clamp-2 text-sm text-[#CBD5E1]">{item.comment_text || item.content || '暂无上下文'}</p>
                </button>
              ))}
            </div>
          )}
        ...
        <Panel title="回复生成器" description="先生成和预览话术，再由人工确认外发。">
          <div className="space-y-4">
            <div>
              <FieldLabel>评论/上下文</FieldLabel>
              <textarea value={replyInput} onChange={(e) => setReplyInput(e.target.value)} rows={5} className={inputClass} placeholder="粘贴客户评论或私信上下文" />
            </div>
            ...
            {reply && (
              <div className="rounded-lg border border-[#1E293B] bg-[#0B0F1A] p-4">
                <p className="text-xs text-[#64748B]">AI 建议回复 {replyGrade && `· ${replyGrade}`}</p>
                <p className="mt-2 text-sm leading-6 text-[#F1F5F9]">{reply}</p>
                <div className="mt-4 flex gap-2">
                  <button onClick={confirmDm} className={buttonSecondary}><Send className="h-4 w-4" />预览私信确认</button>
                  <button className={buttonSecondary}>预览跟进入口</button>
                </div>
              </div>
            )}
          </div>
      ...
      <Panel title="转化漏斗" description="用于老板和运营判断线索在哪个阶段流失。">
        {funnelStages.length === 0 ? (
          <EmptyState title="暂无漏斗数据" description="线索状态更新后，这里会展示从发现到成交的阶段分布。" />
        ) : (
