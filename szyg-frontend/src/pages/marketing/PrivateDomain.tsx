...
function leadName(item?: LeadItem | null): string {
  return String(item?.user_name || item?.author_name || translateCurrent("潜在线索"))
}
...
export default function PrivateDomain() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : t("私域营销数据加载失败"))
    } finally {
      setLoading(false)
    }
  }
  ...
  const openWechat = async () => {
    ...
    try {
      ...
      showToast(t("微信已打开到前台"))
      await load()
    } catch (e) {
      setError(e instanceof Error ? e.message : t("打开微信失败"))
    } finally {
      setWorking('')
    }
  }
  ...
  const captureInput = async () => {
    ...
    try {
      ...
      showToast(t("微信输入区已识别"))
      await load()
    } catch (e) {
      setError(e instanceof Error ? e.message : t("识别输入区失败"))
    } finally {
      setWorking('')
    }
  }
  ...
  const createDraft = async () => {
    ...
    if (!selected || !text) {
      setError(t("请先生成或填写回复话术"))
      return
    }
    if (!overview?.wechat.calibrated) {
      setError(t("请先识别微信输入区"))
      return
    }
    if (!window.confirm(t("将这段话术记录为微信草稿，发送前仍需要你人工确认。"))) return
    setWorking('draft')
    setError('')
    try {
      ...
      showToast(t("已生成微信草稿记录"))
    } catch (e) {
      setError(e instanceof Error ? e.message : t("写入草稿失败"))
    } finally {
      setWorking('')
    }
  }
  ...
  const markFollowed = async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : t("记录跟进失败"))
    } finally {
      setWorking('')
    }
  }
  ...
  return (
    <PageShell title={t("私域营销")} subtitle={t("AI 销售员工会从线索里判断需求，生成跟进话术，并辅助你在微信里安全承接客户。")} icon={MessageCircle}>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
        <MetricCard label={t("待跟进")} value={metrics.pending} icon={UserCheck} />
        <MetricCard label={t("高意向")} value={metrics.high_intent} icon={Sparkles} tone="green" />
        <MetricCard label={t("已跟进")} value={metrics.followed_today} icon={CheckCircle2} tone="blue" />
        <MetricCard label={t("超时提醒")} value={metrics.overdue} icon={Clock} tone="amber" />
        <div className={`rounded-lg border p-4 ${wechatTone(wechat?.state)}`}>
          <p className="text-xs opacity-80">{t("当前电脑微信")}</p>
          <p className="mt-2 text-lg font-semibold">{wechat?.label || t("未连接")}</p>
          <p className="mt-3 text-xs opacity-75">{t("只辅助草稿和跟进，发送前仍需确认。")}</p>
        </div>
      </div>

      <div className="grid gap-5 xl:grid-cols-[340px_minmax(0,1fr)_390px]">
        <Panel title={t("待跟进队列")} description={t("来自智能截流和机会监听的潜在线索。")}>
          {loading ? (
            <div className="flex h-56 items-center justify-center text-[#64748B]"><Loader2 className="h-6 w-6 animate-spin" /></div>
          ) : items.length === 0 ? (
            <EmptyState title={t("暂无线索")} description={t("智能截流或机会监听识别到客户后，会进入这里。")} />
          ) : (
              ...
              {items.map((item, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <p className="mt-3 line-clamp-3 text-sm leading-6 text-[#CBD5E1]">{leadText(item) || t("暂无客户上下文")}</p>
                  </button>
                )
              })}
            </div>
        ...
        <Panel title={t("客户需求判断")} description={t("先判断客户想要什么，再决定是否进入微信跟进。")}>
          {!selected ? (
            <EmptyState title={t("请选择线索")} description={t("选择一条线索后查看来源、意向和推荐动作。")} />
          ) : (
              ...
              <div className="rounded-lg border border-[#1E293B] bg-[#0B0F1A] p-5">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-[#64748B]">{platformLabel(selected.platform)} · {selected.source_title || t("来源内容待补充")}</p>
                  </div>
                  <span className={`rounded-full border px-3 py-1 text-xs ${scoreTone(Number(selected.lead_score || selected.score || 0))}`}>
                    {isHighIntent(selected) ? t("高意向") : t("待确认")} · {selected.grade || selected.lead_score || selected.score || t("未评分")}
                  </span>
                </div>
                <div className="mt-5 rounded-lg border border-[#334155] bg-[#020617] p-4">
                  <p className="text-xs text-[#64748B]">{t("客户原始表达")}</p>
                  <p className="mt-2 whitespace-pre-wrap text-sm leading-7 text-[#E2E8F0]">{selectedText || t("暂无上下文")}</p>
                </div>
              </div>

              <div className="grid gap-3 md:grid-cols-3">
                <div className="rounded-lg border border-[#1E293B] bg-[#0B0F1A] p-4">
                  <p className="text-xs text-[#64748B]">{t("判断")}</p>
                  <p className="mt-2 text-sm leading-6 text-[#CBD5E1]">{isHighIntent(selected) ? t("建议优先进入微信私域承接") : t("先确认需求，再决定是否跟进")}</p>
                </div>
                <div className="rounded-lg border border-[#1E293B] bg-[#0B0F1A] p-4">
                  <p className="text-xs text-[#64748B]">{t("下一步")}</p>
                  <p className="mt-2 text-sm leading-6 text-[#CBD5E1]">{String(selected.recommended_action || t("生成回复并人工确认"))}</p>
                </div>
                <div className="rounded-lg border border-[#1E293B] bg-[#0B0F1A] p-4">
                  <p className="text-xs text-[#64748B]">{t("最近更新")}</p>
                  <p className="mt-2 text-sm leading-6 text-[#CBD5E1]">{formatDateTime(selected.updated_at || selected.created_at)}</p>
                </div>
              </div>

              <div>
                <FieldLabel>{t("跟进备注")}</FieldLabel>
                <textarea
                  value={notes}
                  onChange={(event) => setNotes(event.target.value)}
                  rows={4}
                  className={inputClass}
                  placeholder={t("记录客户需求、预算、顾虑或下一步动作")}
                />
              </div>
            </div>
          )}
        ...
        <Panel title={t("微信跟进")} description={t("生成话术、打开微信、写入草稿，发送前由你确认。")}>
          <div className="space-y-4">
            <div className={`rounded-lg border p-4 ${wechatTone(wechat?.state)}`}>
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-sm font-medium">{t("当前电脑微信")}</p>
                  <p className="mt-1 text-xs opacity-75">{wechat?.label || t("未连接")}</p>
                </div>
            ...
            <div>
              <FieldLabel>{t("AI 建议话术")} {replyGrade && <span className="text-[#64748B]">· {replyGrade}</span>}</FieldLabel>
              <textarea
                value={reply}
                onChange={(event) => setReply(event.target.value)}
                rows={8}
                className={inputClass}
                placeholder={t("生成后可以在这里微调，再写入微信草稿")}
              />
            </div>

            <div>
              <FieldLabel>{t("下次提醒")}</FieldLabel>
