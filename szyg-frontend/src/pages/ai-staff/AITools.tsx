...
const quickFilters: Array<{ key: QuickFilter; label: string; icon: typeof Sparkles }> = [
  { key: 'all', get label() { return translateCurrent("全部工具") }, icon: Sparkles },
  { key: 'hot', get label() { return translateCurrent("热门工具") }, icon: Flame },
  { key: 'latest', get label() { return translateCurrent("最新收录") }, icon: BadgeCheck },
  { key: 'business', get label() { return translateCurrent("企业常用") }, icon: BriefcaseBusiness },
]
...
function toolMatchesSearch(tool: AIToolItem, query: string, t: (key: string) => string) {
  ...
  return [...sourceValues, ...sourceValues.map((value) => t(value))].some((value) => normalize(value).includes(q))
}
...
export default function AITools() {
  ...
  const filteredTools = useMemo(() => {
    return AI_TOOLS_CATALOG
      ...
      .filter((tool) => toolMatchesSearch(tool, search, t))
      .sort((a, b) => {
      ...
      <div className="mx-auto max-w-7xl space-y-5">
        <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="mt-1 max-w-2xl text-body-sm text-[#94A3B8]">
              ...
              {t("为老板和运营员工整理常用外部 AI 工具，快速找到写作、图片、视频、办公、搜索、编程和智能体平台。")}
            </p>
          </div>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            <Metric label={t("精选工具")} value={stats.total} />
            <Metric label={t("热门")} value={stats.hot} />
            <Metric label={t("国内可用")} value={stats.chinaReady} />
            <Metric label={t("企业常用")} value={stats.business} />
          </div>
        </header>
          ...
          <div className="grid gap-3 lg:grid-cols-[1fr_auto] lg:items-center">
            <div className="relative">
              ...
              <input
                ...
                placeholder={t("搜索工具名称、分类、标签或使用场景...")}
        ...
        {filteredTools.length === 0 && (
          <div className="rounded-card-lg border border-[#1E293B] bg-[#111827] px-6 py-12 text-center">
            ...
            <h2 className="mt-3 text-heading-sm text-[#F1F5F9]">{t("没有找到匹配工具")}</h2>
            <p className="mt-1 text-body-sm text-[#64748B]">{t("换一个关键词、分类或快捷筛选试试。")}</p>
          </div>
        )}
      </div>
    </div>
...
function ToolCard({ tool }: { tool: AIToolItem }) {
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <h2 className="truncate text-heading-sm text-[#F1F5F9]">{t(tool.name)}</h2>
          ...
          <div className="mt-1 flex flex-wrap items-center gap-2">
            <span className="rounded-md bg-[#6366F1]/10 px-2 py-0.5 text-[11px] text-[#C4B5FD]">{t(tool.category)}</span>
            <span className="rounded-md bg-[#0B0F1A] px-2 py-0.5 text-[11px] text-[#94A3B8]">{t(tool.pricing)}</span>
            {tool.chinaReady && <span className="rounded-md bg-[#10B981]/10 px-2 py-0.5 text-[11px] text-[#86EFAC]">{t("国内可用")}</span>}
          </div>
        </div>
      ...
      <p className="mt-3 line-clamp-2 text-body-sm leading-6 text-[#CBD5E1]">{t(tool.description)}</p>
      <div className="mt-3 rounded-lg border border-[#1E293B] bg-[#0B0F1A] px-3 py-2">
        <div className="text-[11px] text-[#64748B]">{t("适合场景")}</div>
        <div className="mt-1 line-clamp-2 text-body-xs text-[#94A3B8]">{t(tool.scenario)}</div>
      </div>
