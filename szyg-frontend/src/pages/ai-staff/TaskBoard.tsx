...
const FILTER_TABS = [
  ...
  { key: 'running', label: '⏳ 进行中', icon: null },
  { key: 'needs_human', label: '✋ 需人工', icon: null },
  { key: 'completed', label: '✅ 已完成', icon: null },
  { key: 'failed', label: '❌ 失败', icon: null },
] as const

const COLUMNS = [
  { key: 'running', title: '进行中', icon: Clock, color: '#10B981', bgColor: 'rgba(16,185,129,0.06)' },
  { key: 'needs_human', title: '需人工', icon: Hand, color: '#F59E0B', bgColor: 'rgba(245,158,11,0.06)' },
  { key: 'completed', title: '已完成', icon: CheckCircle2, color: '#64748B', bgColor: 'rgba(100,116,139,0.06)' },
  { key: 'failed', title: '失败', icon: XCircle, color: '#EF4444', bgColor: 'rgba(239,68,68,0.06)' },
] as const
          ...
          <div className="mb-3">
            <div className="flex items-start gap-1.5">
              ...
              <p className="text-xs text-[#F59E0B] leading-relaxed">
                {truncate(task.error || task.progress || '需要人工处理', 60)}
              </p>
            </div>
          </div>
        )}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-[#64748B] mt-0.5">
                {detail?.platform} · 优先级 {detail?.priority}
              </p>
            </div>
          </div>
          <button
          ...
          {loading ? (
            ...
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-3">
                <InfoBlock label="任务类型" value={detail.type_label} />
                <InfoBlock label="触发方式" value={detail.trigger_type} />
                <InfoBlock label="创建时间" value={new Date(detail.created_at).toLocaleString('zh-CN')} />
                <InfoBlock label="状态" value={
                  <span className={cn(
                    ...
                    {detail.status === 'running' ? '进行中' : detail.status === 'needs_human' ? '需人工' : detail.status === 'completed' ? '已完成' : '失败'}
                  </span>
                } />
              </div>
              ...
              {detail.error && (
                <div className="bg-[rgba(239,68,68,0.08)] border border-[rgba(239,68,68,0.2)] rounded-xl p-4">
                  <p className="text-xs font-medium text-[#EF4444] mb-1">错误原因</p>
              ...
              {detail.execution_run?.error_code && (
                <div className="bg-[#0D1321] border border-[#1E293B] rounded-xl p-4">
                  <p className="text-xs font-medium text-[#F59E0B] mb-1">错误码</p>
              ...
              {detail.result && (
                <div className="bg-[rgba(16,185,129,0.06)] border border-[rgba(16,185,129,0.15)] rounded-xl p-4">
                  <p className="text-xs font-medium text-[#10B981] mb-1">执行结果</p>
                      ...
                      <div key={step.id} className="rounded-lg bg-[#0D1321] border border-[#1E293B] px-3 py-3">
                        <div className="flex items-center justify-between gap-3">
                          ...
                          <span className={cn(
                            ...
                            {step.status === 'needs_human' ? '需人工' : step.status}
                          </span>
                        </div>
                        <div className="mt-2 flex flex-wrap gap-3 text-xs text-[#64748B]">
                          <span>{step.duration_ms ? `${Math.round(step.duration_ms / 1000)}s` : '未完成'}</span>
              ...
              {detail.logs.length > 0 && (
                <div>
                  <h4 className="text-sm font-semibold text-[#F1F5F9] mb-3">审计日志</h4>
            ...
            <div className="text-center py-12 text-[#64748B] text-sm">无法加载任务详情</div>
          )}
        </div>
      </motion.div>
    </motion.div>
              ...
              return (
                ...
                >
                  ...
                  <div className="space-y-3">
                    ...
                    {colTasks.length === 0 && (
                      <div className="text-center py-10 text-sm text-[#64748B]">暂无任务</div>
                    )}
                  </div>
                </div>
              )
