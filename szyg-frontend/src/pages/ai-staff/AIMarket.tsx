...
export default function AIMarket() {
  ...
  const localeQuery = useMemo(() => `locale=${encodeURIComponent(locale)}`, [locale])
  // ── 视图状态 ──
  ...
  useEffect(() => {
    setLoading(true)
    apiGet<MarketInitResponse>(`/api/agency/market-init?${localeQuery}`)
      .then((data) => {
      ...
      .catch((e) => setError(getErrorMessage(e, t("加载失败"))))
      .finally(() => setLoading(false))
  }, [localeQuery, t])

  // ── 加载激活专家（用于 handleSelectExpert 后刷新）──
  const loadActive = useCallback(() => {
    apiGet<ActiveExpert | null>(`/api/agency/active?${localeQuery}`)
  ...
  const loadConversations = useCallback(() => {
    apiGet<AgencyConversation[]>(`/api/agency/conversations?${localeQuery}`)
  ...
  useEffect(() => {
    ...
    const doFetch = () => {
      ...
      fetch(url, { headers, signal: abort.signal })
        ...
        .catch((e) => {
          if ((e as Error).name !== 'AbortError') setError(getErrorMessage(e, t("加载专家失败")))
        })
        .finally(() => setLoading(false))
    }
  ...
  }, [locale, selectedDivision, search, view])
  ...
  const handleSelectExpert = useCallback(async (slug: string) => {
    ...
    try {
      ...
      await loadActive()
      const conv = await apiPost<AgencyConversation>(`/api/agency/conversations?${localeQuery}`)
      ...
      setError(getErrorMessage(e, t("激活专家失败")))
    } finally {
      setActivating(false)
    }
  }, [loadActive, loadConversations, localeQuery])
  ...
  const handleOpenConversation = useCallback(async (conv: AgencyConversation) => {
    ...
    try {
      ...
      setError(getErrorMessage(e, t("加载对话失败")))
    } finally {
      setActivating(false)
    }
  }, [loadActive])
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      setError(getErrorMessage(e, t("对话请求失败")))
      fullContent = fullContent || t("抱歉，请求失败了，请重试。")
    } finally {
      setStreaming(false)
    }
  ...
  const deactivateExpert = useCallback(async () => {
    ...
    try {
      ...
      setError(getErrorMessage(e, t("恢复失败")))
    } finally {
      setActivating(false)
    }
  }, [loadActive])
      ...
      <div className="flex flex-1 min-h-0 gap-0">
            ...
            >
              <div className="w-[280px] h-full flex flex-col bg-[#0D1321]">
                <div className="px-4 py-3 border-b border-[#1E293B] flex items-center justify-between">
                  <span className="text-body-md font-medium text-[#F1F5F9]">{t("历史对话")}</span>
                ...
                <div className="flex-1 overflow-y-auto p-2 space-y-1">
                  {conversations.length === 0 ? (
                    <p className="text-body-sm text-[#64748B] text-center py-8">{t("暂无对话记录")}</p>
                  ) : (
                    conversations.map((c) => (
                      <button
                        key={c.id}
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="shrink-0 px-4 py-3 border-b border-[#1E293B] flex items-center gap-3">
            ...
            >
              <ArrowLeft className="w-4 h-4" />  {t("市场")}
            </button>
            ...
            <div className="flex items-center gap-1 shrink-0">
              ...
              {activeExpert && (
                ...
                >
                  <RotateCcw className="w-3.5 h-3.5" />  {t("换专家")}
                </button>
              )}
            </div>
          </div>
          ...
          <div className="flex-1 min-h-0 overflow-y-auto px-4 py-4 space-y-4">
            {messages.length === 0 ? (
              <div className="flex flex-col items-center justify-center min-h-[40vh] text-[#64748B] gap-3">
                ...
                <p className="text-body-md font-medium text-[#F1F5F9]">{expert?.name || t("专家")}</p>
                <p className="text-body-sm text-[#64748B] text-center max-w-sm">
                  {expert?.description || t("开始对话，让专家为你提供专业建议")}
                </p>
              </div>
            ) : (
                ...
                <div key={idx} className={cn('flex gap-3', msg.role === 'user' ? 'justify-end' : 'justify-start')}>
                  ...
                  {msg.role === 'assistant' && !msg.content && streaming && idx === messages.length - 1 ? (
                    <div className="flex items-center gap-2 py-2">
                      ...
                      <span className="text-[13px] text-[#94A3B8]">{statusText || t("思考中…")}</span>
                    </div>
                  ) : (
                    <div className={cn(
          ...
          <div className="shrink-0 border-t border-[#1E293B] bg-[#0D1321]">
            <div className="flex items-end gap-2 px-4 py-3">
              <textarea
                ...
                placeholder={expert ? t("向{v0}提问…", { v0: expert.name }) : t("输入消息…")}
                disabled={streaming}
                rows={1}
    ...
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-6">
      ...
      <motion.div variants={cardVariants}>
        <p className="text-body-lg text-[#94A3B8]">
          ...
          <span className="block text-body-xl font-semibold text-[#F1F5F9]">{t("AI人才市场")}</span>
          <span className="mt-1 block">{t("发现并启用领域专家，为超级员工切换专业角色 · 共")} {totalAgents}  {t("位专家")}</span>
        </p>
      </motion.div>
      ...
      {activeExpert && (
        ...
        >
          ...
          <div className="flex items-center gap-2 shrink-0">
            <span className="flex items-center gap-1 text-[12px] text-[#10B981]"><Check className="w-3.5 h-3.5" />  {t("已激活")}</span>
            <button onClick={deactivateExpert} disabled={activating} className="flex items-center gap-1.5 px-3 py-1.5 rounded-button text-[13px] text-[#94A3B8] hover:text-[#F1F5F9] hover:bg-[rgba(255,255,255,0.05)] transition-colors disabled:opacity-50">
              <RotateCcw className="w-3.5 h-3.5" />  {t("恢复默认")}
            </button>
          </div>
        </motion.div>
      )}
      ...
      <motion.div variants={cardVariants} className="relative">
        ...
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder={t("搜索专家名称、能力或领域…")} className="w-full h-12 pl-11 pr-4 rounded-card bg-[#0D1321] border border-[#1E293B] text-body-md text-[#F1F5F9] placeholder-[#64748B] focus:outline-none focus:border-[#334155] transition-colors" />
      </motion.div>
      ...
      <motion.div variants={cardVariants} className="flex items-center gap-2 flex-wrap">
        <button onClick={() => setSelectedDivision(null)} className={cn('flex items-center gap-1.5 px-3 py-1.5 rounded-button text-[13px] font-medium transition-all', !selectedDivision ? 'bg-[#6366F1] text-white' : 'bg-[#1A2235] text-[#94A3B8] hover:text-[#F1F5F9] border border-[#1E293B]')}>
          <Tag className="w-3.5 h-3.5" />  {t("全部")} {totalAgents}
        </button>
        {divisions.map((d) => {
      ...
      {loading ? (
        <div className="flex items-center justify-center py-20 text-[#64748B] gap-2">
          <Loader2 className="w-5 h-5 animate-spin" /><span className="text-body-md">{t("加载专家列表…")}</span>
        </div>
      ) : agents.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-[#64748B] gap-2">
          <Search className="w-8 h-8 opacity-40" /><span className="text-body-md">{t("未找到匹配的专家")}</span>
        </div>
      ) : (
              ...
              >
                  ...
                  <div className="flex items-start gap-3 mb-3">
                    ...
                    <div className="flex-1 min-w-0 pt-0.5">
                      <div className="flex items-center gap-2">
                        ...
                        {isActive && <span className="flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[10px] bg-[rgba(16,185,129,0.15)] text-[#10B981] shrink-0"><Check className="w-2.5 h-2.5" />  {t("使用中")}</span>}
                      </div>
                      {div && (
                ...
                <div className="px-5 py-2.5 border-t border-[#1E293B] flex items-center justify-between bg-[rgba(0,0,0,0.15)]">
                  <span className="text-[12px] text-[#64748B] group-hover:text-[#94A3B8] transition-colors">
                    {activating ? t("激活中…") : isActive ? t("继续对话") : t("点击选择此专家")}
                  </span>
