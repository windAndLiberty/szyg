...
const TABS: { key: Tab; label: string; icon: typeof ImageIcon }[] = [
  ...
  { key: 'performance', label: '内容效果', icon: BarChart3 },
]
  ...
  return [
    ...
    ...references.map((ref, index) => {
      ...
      const providerLabel = ref.source === 'upload'
        ? `${ref.type === 'image' ? '图片' : ref.type === 'video' ? '视频' : REFERENCE_LABELS[ref.type]}${counters[ref.type]}`
    ...
    try {
      for (const file of Array.from(files)) {
        ...
        if (!type || !allowedTypes.includes(type)) {
          throw new Error(uploadMode === 'image' ? '图片生成仅支持 JPG、PNG 参考图' : '视频生成仅支持 JPG、PNG、WEBP、MP4、MOV 素材')
        }
        if (uploadMode === 'image' && !['jpg', 'jpeg', 'png'].includes(suffix)) {
          throw new Error('图片生成仅支持 JPG、PNG 参考图')
        }
        ...
        if (uploadMode === 'image' && uploadedImages >= 10) throw new Error('图片生成最多添加 10 张参考图')
        if (uploadMode === 'video' && type === 'image' && uploadedImages >= 9) throw new Error('视频生成最多添加 9 张参考图')
        if (uploadMode === 'video' && type === 'video' && uploadedVideos >= 3) throw new Error('视频生成最多添加 3 段参考视频')
        if (type === 'image' && file.size > 15 * 1024 * 1024) throw new Error('单张参考图片不能超过 15MB')
        if (type === 'video' && file.size > 50 * 1024 * 1024) throw new Error('单段参考视频不能超过 50MB')
        const item = await uploadContentReference(file, uploadMode)
        next = [...next, uploadedReference(item)]
        onReferencesChange(next)
      }
    } catch (error) {
      showMessage(getErrorMessage(error, '参考素材上传失败'))
    } finally {
      setUploading(false)
    }
    ...
    >
      ...
      <div className="flex items-stretch">
        {uploadMode && (
          <label className={`group m-3 mr-0 flex w-[76px] shrink-0 cursor-pointer flex-col items-center justify-center gap-1.5 rounded-lg border border-dashed text-center transition-colors ${uploading ? 'cursor-wait border-[#6366F1]/40 bg-[#6366F1]/10 text-[#A5B4FC]' : 'border-[#334155] bg-[#111827] text-[#64748B] hover:border-[#6366F1]/70 hover:text-[#C4B5FD]'}`}>
            ...
            <span className="text-[11px]">{uploading ? '上传中' : '参考内容'}</span>
            <input
              type="file"
              multiple
              disabled={uploading}
      ...
      {(message || uploadMode) && (
        <div className={`border-t border-[#1E293B] px-3 py-2 text-body-xs ${message ? 'text-[#FCA5A5]' : 'text-[#64748B]'}`}>
          {message || (uploadMode ? `${REFERENCE_UPLOAD_RULES[uploadMode].hint}；也可将文件拖到此处` : '')}
        </div>
      )}
    </div>
  )
