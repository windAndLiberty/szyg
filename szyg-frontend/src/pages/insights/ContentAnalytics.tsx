...
const ranges = [{ value: 7, label: '7天' }, { value: 30, label: '30天' }, { value: 90, label: '90天' }]

const typeLabels: Record<string, string> = {
  image: '图片', copy: '文案', text: '文案', video: '视频', voice: '语音', audio: '语音', graphic: '图文', unknown: '其他',
}
...
export default function ContentAnalytics() {
  ...
  useEffect(() => {
    ...
    loadContentInsights(days)
      ...
      .catch((cause) => { if (active) setError(cause instanceof Error ? cause.message : '内容洞察加载失败') })
      .finally(() => { if (active) setLoading(false) })
    return () => { active = false }
  }, [days])

  return <PageShell title="内容洞察" subtitle="观察内容从生成、采纳到真实发布的转化，并与外部市场机会对照。" icon={TrendingUp}>
    ...
    {loading && !data ? <div className="flex h-64 items-center justify-center text-[#64748B]"><Loader2 className="h-7 w-7 animate-spin" /></div> : data && <>
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard label="生成内容" value={formatNumber(data.summary.generated)} icon={Sparkles} hint={`最近 ${days} 天`} />
        <MetricCard label="已采纳" value={formatNumber(data.summary.adopted)} icon={CheckCircle2} tone="green" hint={`采纳率 ${percent(data.summary.adoption_rate)}`} />
        <MetricCard label="提交发布" value={formatNumber(data.summary.published)} icon={Send} tone="blue" hint="按账号级发布任务统计" />
        <MetricCard label="发布成功率" value={percent(data.summary.publish_success_rate)} icon={TrendingUp} tone="amber" hint={`${data.summary.publish_success} 次成功`} />
      </div>

      <Panel title="内容流转" description="同一时间范围内各阶段的真实数量，不把缺失数据补成转化。">
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        ...
        <Panel title="发布平台" description="展示发布任务分布，不代表平台作品表现。">
          {data.platforms.length ? <div className="space-y-4">{data.platforms.map((item) => {
            ...
            return <div key={item.name}><div className="mb-2 flex justify-between text-sm"><span className="text-[#CBD5E1]">{platformLabel(item.name)}</span><span className="text-[#64748B]">{item.value} 次</span></div><div className="h-2 rounded-full bg-[#0B0F1A]"><div className="h-2 rounded-full bg-[#38BDF8]" style={{ width: `${Math.max(6, item.value * 100 / max)}%` }} /></div></div>
          })}</div> : <EmptyState title="暂无发布记录" description="从素材管理与发布提交内容后会出现平台分布。" />}
        </Panel>
      </div>

      <div className="grid gap-5 xl:grid-cols-[1.1fr_0.9fr]">
        <Panel title="最近生成" description="生成内容本身与采纳状态。">
          {data.recent_items.length ? <div className="divide-y divide-[#1E293B]">{data.recent_items.slice(0, 8).map((item) => {
            ...
            return <div key={item.id} className="flex items-center gap-3 py-3 first:pt-0 last:pb-0"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-[#273449] text-[#94A3B8]"><Icon className="h-4 w-4" /></span><div className="min-w-0 flex-1"><p className="truncate text-sm text-[#E2E8F0]">{item.title}</p><p className="mt-1 text-xs text-[#64748B]">{typeLabels[item.type] || item.type} · {formatDateTime(item.created_at)}</p></div><span className={`rounded-full border px-2 py-1 text-[11px] ${item.adopted ? 'border-[#10B981]/25 bg-[#10B981]/8 text-[#6EE7B7]' : 'border-[#334155] text-[#64748B]'}`}>{item.adopted ? '已采纳' : '未采纳'}</span></div>
          })}</div> : <EmptyState title="暂无内容" description="内容生成记录会自动进入这里。" />}
        </Panel>

        <Panel title="市场内容机会" description="由市场情报提供，结合企业发布覆盖计算。">
          {data.market_opportunities.length ? <div className="space-y-3">{data.market_opportunities.slice(0, 4).map((item) => <div key={item.topic} className="rounded-lg border border-[#1E293B] bg-[#0B0F1A] p-4"><div className="flex items-center justify-between gap-3"><p className="text-sm font-medium text-[#E2E8F0]">{item.topic}</p><span className="text-xs text-[#FBBF24]">缺口 {item.gap}</span></div><p className="mt-2 text-xs leading-5 text-[#64748B]">{item.recommendation}</p></div>)}</div> : <EmptyState title="暂无市场机会" description="完成一次市场情报分析后，相关主题会进入内容洞察。" />}
        </Panel>
      </div>
