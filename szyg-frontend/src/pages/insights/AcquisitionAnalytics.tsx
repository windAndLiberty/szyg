...
export default function AcquisitionAnalytics() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        setError(e instanceof Error ? e.message : t("获客洞察数据加载失败"))
      } finally {
        setLoading(false)
      }
    }
  ...
  return (
    <PageShell title={t("获客洞察")} subtitle={t("复盘目标发现、评论执行、线索识别和转化阶段表现。")} icon={TrendingUp}>
      ...
      {loading ? (
        ...
        <>
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            <MetricCard label={t("线索总数")} value={formatNumber(leads.length)} icon={Users} />
            <MetricCard label={t("高意向线索")} value={formatNumber(highIntent)} icon={Target} tone="green" />
            <MetricCard label={t("评论成功率")} value={successRate} icon={MessageSquare} tone="blue" />
            <MetricCard label={t("待处理队列")} value={formatNumber(queued)} icon={AlertTriangle} tone="amber" />
          </div>

          <div className="grid gap-5 xl:grid-cols-2">
            <Panel title={t("获客漏斗")} description={t("从线索发现到转化阶段的数量分布。")}>
              {funnelStages.length === 0 ? (
                <EmptyState title={t("暂无漏斗数据")} description={t("线索状态更新后会生成漏斗分布。")} />
              ) : (
            ...
            <Panel title={t("平台来源分布")} description={t("判断哪个平台带来更多线索和机会。")}>
              {platformCounts.length === 0 ? (
                <EmptyState title={t("暂无平台分布")} description={t("线索产生后会按平台聚合。")} />
              ) : (
                <div className="space-y-3">
                  {platformCounts.map(([platform, count]) => (
                    <div key={platform} className="flex items-center justify-between rounded-lg border border-[#1E293B] bg-[#0B0F1A] p-3">
                      ...
                      <span className="text-sm text-[#94A3B8]">{count}  {t("条线索")}</span>
                    </div>
                  ))}
                </div>
              )}
          ...
          <Panel title={t("转化记录")} description={t("最近进入转化链路的记录，用于复盘跟进质量。")}>
            {conversions.length === 0 ? (
              <EmptyState title={t("暂无转化记录")} description={t("客户跟进和状态推进后，会在这里出现转化记录。")} />
            ) : (
                ...
                <table className="w-full text-left text-sm">
                  <thead className="bg-[#0B0F1A] text-xs text-[#64748B]">
                    <tr>
                      ...
                      <th className="px-3 py-3">{t("阶段")}</th>
                      <th className="px-3 py-3">{t("更新时间")}</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-[#1E293B]">
                    {conversions.map((item, index) => (
                      <tr key={String(item.id || index)}>
                        <td className="px-3 py-3 text-[#F1F5F9]">{String(item.customer_name || item.user_name || item.name || t("未知客户"))}</td>
