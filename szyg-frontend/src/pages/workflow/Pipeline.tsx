...
export default function Pipeline() {
  ...
  return (
    <WorkflowPage subtitle="选择一项岗位任务，补齐少量设置，系统会按计划持续执行并反馈结果。">
      ...
      <div className="flex flex-col gap-4 border-b border-[#1E293B] pb-6 sm:flex-row sm:items-center sm:justify-between">
        <div><p className="text-sm font-medium text-[#E2E8F0]">把重复工作交给系统持续完成</p><p className="mt-1 text-xs leading-5 text-[#64748B]">描述你的目标，或自行组合已有能力；启用前可以检查和调整每一步。</p></div>
        <div className="flex gap-2"><button onClick={() => setDesigner('manual')} className={secondaryButton}><SlidersHorizontal className="h-4 w-4" />手动创建</button><button onClick={() => setDesigner('ai')} className={primaryButton}><Sparkles className="h-4 w-4" />AI 帮我设计</button></div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <Metric label="正在执行" value={overview.running} icon={Activity} tone="blue" />
        <Metric label="需要处理" value={overview.needs_human} icon={AlertTriangle} tone="amber" />
      </div>

      <Section
        title="工作流方案"
        description="按岗位场景选择，不需要搭建节点或理解技术配置。"
        action={<button onClick={load} className={iconButton} title="刷新"><RefreshCw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`} /></button>}
      >
        <div className="mb-5 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="relative w-full lg:w-72"><Search className="absolute left-3 top-2.5 h-4 w-4 text-[#475569]" /><input value={search} onChange={(e) => setSearch(e.target.value)} className={`${inputClass} pl-9`} placeholder="搜索工作流方案" /></div>
        </div>
        ...
        {loading ? <div className="flex h-48 items-center justify-center"><Loader2 className="h-6 w-6 animate-spin text-[#64748B]" /></div> : filtered.length === 0 ? (
          <Empty title="没有匹配的方案" description="调整分类或关键词后再试。" />
        ) : (
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            {filtered.map((template) => {
              ...
              return (
                <button key={template.id} disabled={!template.available} onClick={() => template.available && openConfigure(template)} className={`${panelClass} group flex min-h-64 flex-col p-5 text-left transition-all ${template.available ? 'hover:-translate-y-0.5 hover:border-[#4F46E5] hover:bg-[#151D2E]' : 'cursor-not-allowed opacity-65'}`}>
                  ...
                  <div className="mt-auto pt-5"><p className="text-xs text-[#64748B]">完成后得到</p><p className="mt-1 text-sm text-[#CBD5E1]">{template.outcome}</p><div className="mt-4 flex items-center justify-between text-xs text-[#818CF8]"><span>{template.steps.length} 个标准步骤</span>{template.available && <ChevronRight className="h-4 w-4 transition-transform group-hover:translate-x-0.5" />}</div></div>
                </button>
              )
            })}
          </div>
      ...
      <Section title="我的工作流" description="已启用的方案会按照计划运行，也可以随时暂停或立即执行。">
        {instances.length === 0 ? <Empty title="还没有启用方案" description="从上方选择一个岗位任务，完成简单设置后即可开始。" /> : (
          <div className="overflow-x-auto">
            <table className="w-full min-w-[820px] text-left text-sm">
              <thead className="text-xs text-[#64748B]"><tr className="border-b border-[#1E293B]"><th className="pb-3 font-medium">方案</th><th className="pb-3 font-medium">运行计划</th><th className="pb-3 font-medium">下次运行</th><th className="pb-3 font-medium">最近结果</th><th className="pb-3 font-medium">状态</th><th className="pb-3 text-right font-medium">操作</th></tr></thead>
              <tbody className="divide-y divide-[#1E293B]">
                {instances.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td className="py-4">{item.last_run ? <WorkflowStatus status={item.last_run.status} /> : <span className="text-xs text-[#64748B]">尚未运行</span>}</td>
                    <td className="py-4"><WorkflowStatus status={item.status} /></td>
                    <td className="py-4"><div className="flex justify-end gap-2"><button onClick={() => runNow(item)} disabled={working === `run:${item.id}`} className={iconButton} title="立即运行">{working === `run:${item.id}` ? <Loader2 className="h-4 w-4 animate-spin" /> : <Play className="h-4 w-4" />}</button><button onClick={() => changeStatus(item)} disabled={working === item.id} className={iconButton} title={item.status === 'active' ? '暂停' : '继续'}>{item.status === 'active' ? <Pause className="h-4 w-4" /> : <Play className="h-4 w-4" />}</button><button onClick={() => remove(item)} className={iconButton} title="移除"><Trash2 className="h-4 w-4" /></button></div></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {selected && (
        <div className="fixed inset-0 z-50 flex justify-end bg-black/55" onMouseDown={(e) => { if (e.currentTarget === e.target) setSelected(null) }}>
          <aside className="h-full w-full max-w-xl overflow-y-auto border-l border-[#273449] bg-[#0F1624] shadow-2xl">
            <div className="sticky top-0 z-10 flex items-start justify-between border-b border-[#273449] bg-[#0F1624] px-6 py-5"><div><p className="text-xs text-[#818CF8]">{selected.category_label}</p><h2 className="mt-1 text-lg font-semibold text-[#F1F5F9]">启用 {selected.name}</h2><p className="mt-2 text-sm leading-6 text-[#94A3B8]">{selected.description}</p></div><button onClick={() => setSelected(null)} className={iconButton}><X className="h-4 w-4" /></button></div>
            <div className="space-y-6 p-6">
              <div><label className="mb-2 block text-xs text-[#94A3B8]">方案名称</label><input value={name} onChange={(e) => setName(e.target.value)} className={inputClass} /></div>
              <div><label className="mb-2 block text-xs text-[#94A3B8]">运行频率</label><div className="grid grid-cols-3 gap-2">{(['manual', 'daily', 'weekly'] as WorkflowSchedule['type'][]).map((value) => <button key={value} onClick={() => setScheduleType(value)} className={`rounded-lg border px-3 py-2.5 text-sm ${scheduleType === value ? 'border-[#6366F1] bg-[#6366F1]/12 text-[#C7D2FE]' : 'border-[#334155] text-[#94A3B8]'}`}>{value === 'manual' ? '手动运行' : value === 'daily' ? '每天' : '每周'}</button>)}</div>{scheduleType !== 'manual' && <input type="time" value={time} onChange={(e) => setTime(e.target.value)} className={`${inputClass} mt-3`} />}</div>
              ...
              <div><p className="mb-3 text-xs font-medium text-[#94A3B8]">标准步骤</p><div className="space-y-2">{selected.steps.map((step, index) => <div key={step.id} className="flex gap-3 rounded-lg border border-[#273449] bg-[#0B0F1A] p-3"><div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[#6366F1]/15 text-xs text-[#A5B4FC]">{index + 1}</div><div><p className="text-sm text-[#E2E8F0]">{step.name}</p><p className="mt-1 text-xs leading-5 text-[#64748B]">{step.description}</p>{step.requires_confirmation && <p className="mt-2 inline-flex items-center gap-1 text-xs text-[#FBBF24]"><AlertTriangle className="h-3 w-3" />执行前需要确认</p>}</div></div>)}</div></div>
              <div className="flex gap-3 border-t border-[#273449] pt-5"><button onClick={() => setSelected(null)} className={`${secondaryButton} flex-1`}>取消</button><button onClick={create} disabled={!name.trim() || working === 'create'} className={`${primaryButton} flex-1`}>{working === 'create' ? <Loader2 className="h-4 w-4 animate-spin" /> : <Plus className="h-4 w-4" />}启用方案</button></div>
            </div>
          </aside>
        </div>
      )}
      {designer && <WorkflowDesigner mode={designer} capabilities={capabilities} onClose={() => setDesigner(null)} onActivated={async () => { setDesigner(null); setToast('工作流方案已启用'); window.setTimeout(() => setToast(''), 2600); await load() }} />}
    </WorkflowPage>
  )
}
