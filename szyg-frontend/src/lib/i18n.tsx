...
import { autoUiEnUS } from '@/locales/autoUi'
...
const zhMessages: Record<string, string> = { ...coreZhCN, ...digitalHumanZhCN }
const enMessages: Record<string, string> = { ...coreEnUS, ...digitalHumanEnUS }

function translate(locale: AppLocale, key: string, values?: InterpolationValues) {
  const coreMessages = locale === 'en-US' ? enMessages : zhMessages
  ...
  if (locale === 'en-US' && autoUiEnUS[key]) return interpolate(autoUiEnUS[key], values)
  return interpolate(key, values)
}

/** Locale-aware translation for formatters and event helpers outside React components. */
export function translateCurrent(key: string, values?: InterpolationValues) {
...
export function I18nProvider({ children }: { children: ReactNode }) {
  ...
  const translateMessage = useCallback((key: string, values?: InterpolationValues) => translateCurrent(key, values), [])
  const value = useMemo<I18nContextValue>(() => ({
    locale, setLocale: setLocaleState, t: translateMessage,
