...
| Windows | `C:\szyg\` | 运行环境，自动同步自 Linux |

> **Hermes 内核**: 位于 `server/` 中（嵌入运行时）。上游: Nous Research hermes-agent v0.15.1 (MIT)。
> 详见 `server/HERMES_UPSTREAM.md`。
...
scp /tmp/szyg-sync.tar.gz Administrator@192.168.1.105:C:/ && \
ssh Administrator@192.168.1.105 "cd C:\szyg && python -c \"import tarfile; tarfile.open('C:/szyg-sync.tar.gz').extractall()\""
```
...
| 优先级 | 功能 | 说明 |
|--------|------|------|
| P0 | 白屏问题确认 | 用户验证新 SPA 是否正常渲染 |
| P0 | Chrome App 模式验证 | 双击桌面快捷方式完整流程 |
| P1 | ComfyUI 启动 | 启动 ComfyUI 服务，测试生图 |
| P1 | FFmpeg 配置 | Windows 安装 FFmpeg，测试视频处理 |
| P2 | Electron 原生壳 | 等网络恢复后安装 electron |
| P2 | 代码签名 | 购买证书签名 .exe |
