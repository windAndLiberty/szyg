...
$runtimeRequirements = Join-Path $repoRoot "server\hermes-runtime-requirements.txt"
...
& $python -m pip install --disable-pip-version-check -r $runtimeRequirements
if ($LASTEXITCODE -ne 0) {
    throw "Unable to install the SZYG Hermes runtime dependencies"
}
